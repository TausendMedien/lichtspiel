...
export const heatMap: Pattern = {
  ...
  controls: [
    {
      ...
      tip: "Amplify the camera signal — higher values make motion visible with less physical movement.",
      get: () => gainParam,
    ...
    },
    {
      ...
      tip: "Minimum pixel change required to register as motion. Higher = only strong movements appear.",
      get: () => thresholdParam,
  ...
  activate() {
    // Heat Map IS the camera — auto-enable so the user doesn't have to turn it on manually.
