...
// Heat state — DataTexture Sobel bends stripe phase locally (not base coord)
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Heat Sobel: shifts stripe phase locally — lines bend at body edges, not globally
    if (uHeatStrength > 0.001) {
      ...
      psi += ((hR - hL) * 0.7 + (hU - hD) * 0.3) * uHeatStrength * 0.5;
    }
...
export const flowLines: Pattern = {
  ...
  controls: [
    { label: "Line Count",  type: "range", min: 10,  max: 150,  step: 1,    default: 124,  tip: "Number of flow lines drawn each frame.",                             get: () => lineCount,   set: (v) => { lineCount = v; } },
    { label: "Flow Speed",  type: "range", min: 0.0, max: 0.20, step: 0.01, default: 0.02, audioWeight: 0.35, tip: "How fast lines travel along the noise field.",             get: () => flowSpeed,   set: (v) => { flowSpeed = v; } },
    { label: "Warp Amount", type: "range", min: 0.0, max: 3.0,  step: 0.05, default: 0.5,  tip: "How much the noise field distorts the lines.",                       get: () => warpAmount,  set: (v) => { warpAmount = v; } },
    ...
    { label: "Heat Strength", type: "range", min: 0, max: 2.5, step: 0.1, default: 1.8, interactive: 'heat' as const, tip: "How much heat-map motion bends flow lines locally. Requires Heat.", get: () => heatStrength, set: v => { heatStrength = v; } },
  ...
  dispose() {
    ...
    accTime = 0; warpDisplay = warpAmount;
  },
};
